...
/*   Updated: 2025/08/27 17:17:09 by mfassad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_top_bottom_walls(t_game *game)
{
	...
	while (x < game->width)
	{
		if (game->map[0][x] != '1' || game->map[game->height - 1][x] != '1')
			error_and_exit("ERROR: Map must be enclosed by walls", game);
		x++;
	}
}
...
static void	check_rectangular(t_game *game)
{
	...
	while (y < game->height)
	{
		...
		while (game->map[y][len])
			len++;
		if (len != game->width)
			error_and_exit("ERROR: Map must be rectangular", game);
		y++;
	}
}
	...
	while (y < game->height)
	{
		...
		while (x < game->width)
		{
			...
			if (ch != '0' && ch != '1' && ch != 'P' && ch != 'E' && ch != 'C')
				error_and_exit("ERROR: Invalid map character", game);
			else if (ch == 'P')
				(*p)++;
			else if (ch == 'E')
				(*e)++;
...
int	validate_map(t_game *game)
{
	...
	if (p != 1 || e < 1 || c < 1)
		error_and_exit("ERROR:contain 1 player,at least 1 exit/colletta", game);
	game->collectibles = c;
	return (1);
}
